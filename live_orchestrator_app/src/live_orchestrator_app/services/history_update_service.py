...
class HistoryUpdateService:
    ...
    async def update_histories(self, db_session: AsyncSession, match_id: int) -> None:
        try:
            ...
            update_history_task: List[AsyncTask[str, Any]] = [
                ...
                AsyncTask(
                    key="update_player_hero_histories",
                    coro=self._update_player_hero_histories(history_repository, match_details, match_outcome),
                ),
            ]
    ...
    async def _update_team_histories(
        self, history_repository: HistoryRepository, match_details: MatchTable, match_outcome: MatchOutcomeTable
    ) -> None:
        team_tasks = [
            AsyncTask(
