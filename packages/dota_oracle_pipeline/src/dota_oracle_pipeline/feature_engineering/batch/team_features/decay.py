...
class TeamDecayFeatureGenerator:
    ...
    Bayesian smoothing is applied using prior_mean and prior_count:
    ...
    def generate(
        ...
        *,
        ...
        for match in matches:
            ...
            radiant_wr = self._read_wr_decay(team_states.get(rad_id), now_ts, prior_mean, prior_count, half_life_days)
            dire_wr = self._read_wr_decay(team_states.get(dire_id), now_ts, prior_mean, prior_count, half_life_days)
            matchup_wr = self._read_matchup_wr_decay(
                matchup_states, rad_id, dire_id, now_ts, prior_mean, prior_count, half_life_days
            )
    ...
    def _read_wr_decay(
        ...
        if (games + prior_count) == 0:
            return None
        return (wins + prior_count * prior_mean) / (games + prior_count)
    ...
    def _read_matchup_wr_decay(
        ...
        if (games + prior_count) == 0:
            return None
        t1_win_rate = (t1_wins + prior_count * prior_mean) / (games + prior_count)
