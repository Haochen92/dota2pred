...
from dota_oracle.models.pipeline import PredictionWorkItem
        ...
        try:
            ...
            async with AsyncSession(self.engine) as session:
                async with session.begin():
                    ...
                    input_array = await self.feature_preparation_service.prepare_features_for_inference(match_id, session)
                    ...
                    await self.match_prediction_service.predict_and_store(
                        db_session=session,
                        match_id=match_id,
                        input_array_for_inference=input_array
                    )
                    
                    logger.debug(f"Successfully processed prediction for event '{event_id}', match_id={match_id}")
        except Exception as e:
            logger.error(
                f"Failed to process prediction for event '{event_id}', match_id={match_id}: {e}",
                exc_info=True
            )
            raise e