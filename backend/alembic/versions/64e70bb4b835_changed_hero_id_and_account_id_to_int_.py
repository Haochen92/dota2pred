...
def upgrade() -> None:
    ...
    op.alter_column('match_predictions', 'prediction_date',
               ...
               postgresql_using="prediction_date AT TIME ZONE 'UTC'") # Assume existing naive were UTC

    op.create_index(op.f('ix_match_predictions_prediction_date'), 'match_predictions', ['prediction_date'], unique=False, if_not_exists=True)
    op.create_index(op.f('ix_match_predictions_predictor_name'), 'match_predictions', ['predictor_name'], unique=False, if_not_exists=True)
    ...
    for slot in hero_id_slots:
        op.alter_column('matches', f'slot_{slot}_hero_id',
    ...
    for table_name in history_tables:
        ...
        op.alter_column(table_name, 'win',
                   existing_type=sa.BOOLEAN(),
                   nullable=True) # Consistent with Optional[bool] in model
        
        # Convert 'start_time' from float (epoch) to TIMESTAMPTZ
        op.alter_column(table_name, 'start_time',
                   existing_type=sa.DOUBLE_PRECISION(precision=53),
                   type_=sa.TIMESTAMP(timezone=True),
    ...
    op.create_foreign_key(
        ...
        ["match_id"], ["match_id"] # Assuming match_outcomes.match_id is also BigInt now
    )
    op.create_foreign_key(
        "fk_match_predictions_match_id",
        "match_predictions", "matches",
        ["match_id"], ["match_id"] # Assuming match_predictions.match_id is also BigInt
    )
...
def downgrade() -> None:
    ...
    for table_name in reversed(history_tables): # Process in reverse for downgrade
        ...
        op.alter_column(table_name, 'win',
                   existing_type=sa.BOOLEAN(),
                   nullable=False) # IMPORTANT: Revert to original nullability. If it was NOT NULL, set False.
    ...
    op.add_column('match_predictions', sa.Column('name', sa.VARCHAR(), autoincrement=False, nullable=True)) # Assuming VARCHAR & nullable
    op.add_column('match_predictions', sa.Column('version', sa.VARCHAR(), autoincrement=False, nullable=True)) # Assuming VARCHAR & nullable
    ...
    op.alter_column('match_predictions', 'prediction_date',
               ...
               postgresql_using="prediction_date AT TIME ZONE 'UTC'") # Get naive representation of UTC moment
    ...
    op.create_foreign_key(
        'match_outcomes_match_id_fkey', # Original name
        'match_outcomes', 'matches',
        ['match_id'], ['match_id']
    )
    # ### end Alembic commands ###