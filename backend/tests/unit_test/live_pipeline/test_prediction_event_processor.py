...
async def test_process_event_successfully(prediction_event_processor, mock_async_session, mocker):
    ...
    mock_predict_and_store.assert_awaited_once_with(
        ...
        input_array_for_inference=mock_input_array
    )
...
async def test_process_event_feature_preparation_returns_none(prediction_event_processor, mock_async_session, mocker):
    ...
    # ACT & ASSERT
    with pytest.raises(ValueError, match=f"Feature preparation failed or returned empty features for match {work_item.match_id}"):
        await prediction_event_processor.process_event(work_item)
