...
def test_to_utc_different_timezone_datetime():
...
def test_to_utc_unix_timestamp_seconds():
...
@pytest.mark.parametrize("str_input, fmt_in_code", [
    ("2023-01-01 10:00:00", '%Y-%m-%d %H:%M:%S'),
    ("2023-01-01 10:00:00.123456", '%Y-%m-%d %H:%M:%S.%f'),
    ("2023/01/01 10:00:00", '%Y/%m/%d %H:%M:%S'),
    ("01-01-2023 10:00:00", '%d-%m-%Y %H:%M:%S'),
])
def test_to_utc_custom_format_string(str_input, fmt_in_code):
...
def test_to_utc_unsupported_type_raises_type_error():
    list_input = [2023, 1, 1]
    with pytest.raises(TypeError, match="Unsupported input type for time conversion"):
        to_utc_datetime_object(list_input)
        

def test_to_utc_internal_conversion_failure_raise_value_error(mocker):
    empty_string_input = ""
