...
class TestStoreFeatures:
    ...
    async def test_store_feature_successfully(
        ...
        actual_instance_list = await test_repository._get_data(
            model_class=table_class, id_filters=list(expected_instance_dict.keys())
        )
    ...
    async def test_store_empty_list_handles_gracefully(self, features_repository_test_subject: FeaturesRepository):
