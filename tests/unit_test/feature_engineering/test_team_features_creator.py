...
async def test_team_feature_success(
    ...
    # Mock called methods
    mocker.patch.object(
        team_feature_creator, "_calculate_team_win_rate", return_value=0.6
    )  # version 3.8 auto creates asyncMock
    ...
    result = await team_feature_creator.create_team_features(
        db_session=mock_async_session, match_instances=[match_instance]
    )
