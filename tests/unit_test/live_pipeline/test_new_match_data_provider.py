...
async def test_get_work_items_successfully(new_match_data_provider, ongoing_league_game_factory, mocker) -> None:
    ...
    mock_fetch_current_matches = mocker.patch.object(
        new_match_data_provider, "_fetch_ongoing_matches", return_value=mock_list_ongoing_matches
    )
...
async def test_filter_new_matches_with_values(new_match_data_provider, ongoing_league_game_factory, mocker) -> None:
    ...
    mock_get_new_match_set = mocker.patch.object(
        new_match_data_provider.redis, "update_live_match_set_and_get_new", return_value=mock_new_match_set
    )
    ...
    # Assert
    assert len(actual_new_matches) == len(
        mock_new_match_set
    ), f"Expected {len(mock_new_match_set)}, got {len(actual_new_matches)}"
