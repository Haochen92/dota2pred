...
from dota_oracle_common.utils import TaskRunner
...
async def test_create_and_store_features_sucessfully(
    ...
    mock_logger.debug.assert_called_once_with(f"Successfully stored all features for match {match_instance.match_id}")
...
async def test_missing_feature_raise_error(
    ...
    mock_team_feature_creator.create_team_features.return_value = team_features_table_factory.build(match_id=123)
    mock_player_hero_feature_creator = AsyncMock()
    mock_player_hero_feature_creator.create_player_hero_features.return_value = player_hero_feature_table_factory.build(
        match_id=123
    )
