...
class TestRedisPubSubService:
    ...
    async def test_publish_calls_redis_correctly(self, redis_pubsub_service_subject: RedisPubSubService):
        # ARRANGE

        # Configure the mock's return value
        redis_pubsub_service_subject.redis.publish.return_value = 1
        mock_publish = redis_pubsub_service_subject.redis.publish
        ...
        # ACT
        receivers = await redis_pubsub_service_subject.publish_live_update(channel=test_channel, payload=mock_payload)
    ...
    async def test_publish_retries_and_raises_exception(self, redis_pubsub_service_subject: RedisPubSubService):
        # ARRANGE
        redis_pubsub_service_subject.redis.publish.side_effect = aioredis.ConnectionError("Simulated Failure")
        mock_publish = redis_pubsub_service_subject.redis.publish
        ...
        with pytest.raises(aioredis.ConnectionError):
            await redis_pubsub_service_subject.publish_live_update(channel="any-channel", payload=mock_payload)
    ...
    async def test_listen_to_channel_yield_messages_successfully(
        self, mocker, redis_pubsub_service_subject: RedisPubSubService
    ):
        ...
        async for message in redis_pubsub_service_subject.listen_to_channel("test-channel"):
            received_messages.append(message)
            if len(received_messages) == len(mock_messages):
                break

        mock_get_stream.assert_called_once_with(redis_pubsub_service_subject, "test-channel")
