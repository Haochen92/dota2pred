...
class HierarchicalFeatureTuner:
    ...
    def tune_features(self) -> FeatureHyperparams:
        ...
        self.study_priors.optimize(lambda trial: self._objective_tune_priors(trial, best_timescale_params), n_trials=40)
        ...
        self.study_prior_means.optimize(
            lambda trial: self._objective_tune_prior_means(trial, best_timescale_params, best_prior_params), n_trials=40
        )
    ...
    def _objective_tune_timescales(self, trial: optuna.trial.Trial) -> float:
        ...
        player_half_life = trial.suggest_categorical(
            "player_half_life_days", PLAYER_HERO_DYNAMIC_PRIOR_CONFIG["player_half_life_days"]
        )
        player_hero_hero_half_life = trial.suggest_categorical(
            "player_hero_hero_half_life_days", PLAYER_HERO_DYNAMIC_PRIOR_CONFIG["hero_half_life_days"]
        )

        # 2. Generate features using suggested timescales and fixed defaults for others
        team_features = self.team_decay_generator.generate(
            self.sorted_match_list,
            half_life_days=team_half_life,
            prior_count=self.DEFAULT_PRIOR_COUNT,
            prior_mean=self.DEFAULT_PRIOR_MEAN,
        )
        hero_features = self.hero_winrate_decay_generator.generate(
            self.sorted_match_list,
            half_life_days=hero_half_life,
            prior_count=self.DEFAULT_PRIOR_COUNT,
            prior_mean=self.DEFAULT_PRIOR_MEAN,
        )
    ...
    def _objective_tune_priors(self, trial: optuna.trial.Trial, timescale_params: Dict[str, Any]) -> float:
        ...
        player_prior = trial.suggest_float(
            "player_prior_count", **PLAYER_HERO_DYNAMIC_PRIOR_CONFIG["player_prior_count"], log=True
        )
        player_hero_hero_prior = trial.suggest_float(
            "player_hero_hero_prior_count", **PLAYER_HERO_DYNAMIC_PRIOR_CONFIG["hero_prior_count"], log=True
        )

        # 2. Generate features using fixed timescales, suggested priors, and default mean
        team_features = self.team_decay_generator.generate(
            self.sorted_match_list,
            half_life_days=timescale_params["team_half_life_days"],
            prior_count=team_prior,
            prior_mean=self.DEFAULT_PRIOR_MEAN,
        )
        hero_features = self.hero_winrate_decay_generator.generate(
            self.sorted_match_list,
            half_life_days=timescale_params["hero_half_life_days"],
            prior_count=hero_prior,
            prior_mean=self.DEFAULT_PRIOR_MEAN,
        )
        player_hero_features = self.player_hero_dynamic_prior_generator.generate(
            self.sorted_match_list,
    ...
    def _objective_tune_prior_means(
        self, trial: optuna.trial.Trial, timescale_params: Dict[str, Any], prior_params: Dict[str, Any]
    ) -> float:
        ...
        player_hero_hero_mean = trial.suggest_float(
            "player_hero_hero_prior_mean", **PLAYER_HERO_DYNAMIC_PRIOR_CONFIG["hero_prior_mean"]
        )
        ...
        team_features = self.team_decay_generator.generate(
            self.sorted_match_list,
            half_life_days=timescale_params["team_half_life_days"],
            prior_count=prior_params["team_prior_count"],
            prior_mean=team_mean,
        )
        hero_features = self.hero_winrate_decay_generator.generate(
            self.sorted_match_list,
            half_life_days=timescale_params["hero_half_life_days"],
            prior_count=prior_params["hero_prior_count"],
            prior_mean=hero_mean,
        )
        player_hero_features = self.player_hero_dynamic_prior_generator.generate(
            self.sorted_match_list,
            player_half_life_days=timescale_params["player_half_life_days"],
